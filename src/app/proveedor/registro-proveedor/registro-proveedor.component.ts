...
export class RegistroProveedorComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,private router:Router,private activeRoute: ActivatedRoute, private proveedorInyectable: ProveedorService) { }
  ...
  ngOnInit(): void {
    ...
    this.formularioCrearProveedor = this.formBuilder.group({
      ...
      email: ["", Validators.compose([ Validators.required , Validators.pattern("^[a-zA-Z0-9._\\-]+\\@[a-zA-Z0-9._\\-]+(\\.[a-zA-Z]+)+$") ])],
    ...
    if (id != undefined) {
      ...
      this.proveedorInyectable.octenerProveedor(id,this.token).subscribe(proveedor => {
  ...
  agregar(){
    ...
    this.proveedorInyectable.registarProveedor(this.proveedor,this.token).subscribe((res)=>{
  ...
  modificar(){
    this.proveedor = this.formularioCrearProveedor.value as Proveedor
    this.proveedorInyectable.editarProveedor(this.proveedor,this.idproveedor,this.token).subscribe((res)=>{
